...
/**
 * FTP socket implementation of FTP functions.
 *
...
function ftp_pasv(&$control, $pasv)
{
    ...
    if (isset($GLOBALS['_FTP']['DATA'])) {
        fclose($GLOBALS['_FTP']['DATA']);
        $GLOBALS['_FTP']['DATA'] = null;
    ...
    if (!$pasv) {
        $GLOBALS['_FTP']['USE_PASSIVE'] = false;
        # Pick random "low bit"
        $low = rand(39, 250);
    ...
    $data = fsockopen($ip, $port, $iError, $sError,
        $GLOBALS['_FTP']['timeout']);

    if (is_resource($data)) {
        $GLOBALS['_FTP']['USE_PASSIVE'] = true;
        $GLOBALS['_FTP']['DATA'] = &$data;
        stream_set_blocking($data, true);
        stream_set_timeout($data, $GLOBALS['_FTP']['timeout']);
...
function ftp_rawlist(&$control, $pwd, $recursive = false)
{
    ...
    if (!isset($GLOBALS['_FTP']['DATA']) ||
            !is_resource($GLOBALS['_FTP']['DATA'])) {
        ftp_pasv($control, $GLOBALS['_FTP']['USE_PASSIVE']);
    }
    ...
    $data = &$GLOBALS['_FTP']['DATA'];
    if (!$GLOBALS['_FTP']['USE_PASSIVE']) {
        $data = &socket_accept($data);
    }

    $content = array();

    switch ($GLOBALS['_FTP']['USE_PASSIVE']) {
        case true:
            while (true) {
        ...
        case false:
            ...
            socket_close($GLOBALS['_FTP']['DATA']);
            socket_close($data);
            break;

    }

    $data = $GLOBALS['_FTP']['DATA'] = NULL;
