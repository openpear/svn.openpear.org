...
namespace Net\RouterOS {
    ...
    class Client
    {
         ...
         * @param string $tag The tag of the request to complete.
         * @return array An array with any responses that haven't been passed to
         * a callback function or previously extracted with
         * {@link extractNewResponsesFor}.
         */
        public function completeRequest($tag)
        {
            $isTagNull = null === $tag;
            $result =
                $isTagNull ? array() : $this->extractNewResponsesFor($tag);
            while (0 !== $this->getPendingRequestsCount()) {
                ...
                if ($newReply->getTag() === $tag) {
                    ...
                    if ($newReply->getType() === Response::TYPE_FINAL) {
                        if (!$isTagNull) {
                            $result = array_merge(
                                $result, $this->extractNewResponsesFor($tag)
                            );
                        }
                        break;
                    }
         ...
         * 
         * Gets all replies for a request that haven't been passed to a callback
         * and clears the buffer from them.
         * @param string $tag The tag of the request to extract responses for.
         * @return array An array of {@link Response} objects for the
         * specified request.
         * @see loop()
         */
        public function extractNewResponsesFor($tag)
        {
         ...
         * extracted with {@link extractNewResponsesFor()}.
         ...
         * that {@link extractNewResponsesFor()} can still be used to extract
