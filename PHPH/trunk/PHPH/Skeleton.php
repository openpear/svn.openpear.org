...
class PHPH_Skeleton
{
	...
	{
		$path = dirname(__FILE__)."/Skeleton/config.m4";
		return file_get_contents($path);
	}

	public static function loadConfigW32()
	{
		$path = dirname(__FILE__)."/Skeleton/config.w32";
		return file_get_contents($path);
	}

	public static function loadH()
	{
		$path = dirname(__FILE__)."/Skeleton/php_skeleton.h";
		return file_get_contents($path);
	}

	public static function loadC()
	{
		$path = dirname(__FILE__)."/Skeleton/php_skeleton.c";
		return file_get_contents($path);
	}

	public static function loadMain()
	{
		$path = dirname(__FILE__)."/Skeleton/skeleton.c";
		return file_get_contents($path);
	}

	public static function loadDSP()
	{
		$path = dirname(__FILE__)."/Skeleton/skeleton.dsp";
		return file_get_contents($path);
	}
